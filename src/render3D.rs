...
use crate::render::{Render, width_on_fov};
// Using
use std::rc::Rc;
use num_traits::Zero;
use pixels::Pixels;
...
impl Surface {
    ...
    fn look_and_move_updown(&self, player: &Player) -> (f32, f32) {
        ...
        let factor = (width_on_fov() as f32) / (consts::UPDOWN_FACTOR as f32) - 0.1;
        // Start
    ...
    fn plane_uv(&self, player: &Player, mut x:i32 , mut y:i32 , look_updown: f32, move_updown: f32, tile: f32) -> (f32, f32) {
        ...
        let fy = (width_on_fov() as f32) / z * move_updown * tile;
        let psin = player.sin();
        let pcos = player.cos();
...
impl WallContext {
    ...
    fn project(&mut self, player: &Player, face: &Face, wall2d: &[Vec2<i32>; 2], height: &SectorHeight) -> bool {
        ...
        // Screen position
        for i in 0..4 {
            self.wall[i].x = (self.wall[i].x * width_on_fov()) / self.wall[i].y + consts::H_WIDTH as i32;
            self.wall[i].y = (self.wall[i].z * width_on_fov()) / self.wall[i].y + consts::H_HEIGHT as i32;
        }
        // Draw
        self.visiable = true;
        // Return visiable
